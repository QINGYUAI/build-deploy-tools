...
### 修复
- 🔧 **修复 npm run 参数传递问题** - 支持 `npm run build-copy --build=vam4` 方式传递参数
  - 修复当使用 `npm run script --build=` 时，npm 会将参数转换为 `npm_config_build` 环境变量
  - 修改 `bin/build-copy.js` 和 `lib/utils.js`，添加对 `npm_config_build` 的检查
  - 确保通过 npm run 传递的构建文件名能正确覆盖 `.env` 文件中的 `SOURCE_DIR` 配置
  - 更新文档，说明两种使用 `npm run` 的方式（带双破折号和不带双破折号）

### 改进
- ✨ 优化 npm run 参数处理逻辑，支持更多使用方式
- 📚 更新 README.md，说明 npm run 的两种用法
- 🧪 新增测试脚本 `tests/test-build-arg.js` 和 `tests/test-npm-config.js` 验证参数传递
...
### 新增
- 🚀 **统一高科技消息系统** - 全新的消息格式化引擎
- 🔧 **formatSystemMessage()** - 统一的消息格式化函数
- 🌟 **高科技术语升级** - 专业化的系统操作术语
