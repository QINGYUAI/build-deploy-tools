...
环境变量的优先级顺序（从高到低）：
1. **命令行参数** ⭐ - `--target=`, `--source=`, `--build=` 等（最高优先级，会覆盖其他配置）
2. **npm配置** - `npm_config_*` 环境变量
3. **系统环境变量和 `.env` 文件** - `TARGET_DIR`, `SOURCE_DIR`, `BUILD_NAME` 等
   - 系统环境变量和 `.env` 文件中的变量优先级相同
   - `.env` 文件会在模块加载时自动加载到环境变量中
4. **默认值** - 代码中定义的默认值

**重要**：命令行参数会优先于 `.env` 文件中的配置。如果同时指定了命令行参数和 `.env` 文件，命令行参数会生效。
