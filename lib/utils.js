 ...
 * 这是一个可选功能，如果 dotenv 不存在也不会报错
 * @param {string} cwd - 工作目录，默认为当前工作目录
  ...
  try {
    ...
    if (fs.existsSync(envPath)) {
      ...
      try {
        ...
        // dotenv.config() 返回 { parsed: {...}, error: ... }
        if (result.error) {
          console.warn('⚠️  加载 .env 文件时出错:', result.error.message)
          return false
        }
        
        // 成功加载
        if (result.parsed && Object.keys(result.parsed).length > 0) {
          // 只在开发环境或设置了 DEBUG 时显示
        ...
        // dotenv 不存在或加载失败
        if (dotenvError.code === 'MODULE_NOT_FOUND') {
          // dotenv 未安装，提示用户（可选）
          if (process.env.DEBUG === 'true') {
            console.warn('⚠️  dotenv 未安装，.env 文件不会被加载。安装方式: npm install dotenv')
          }
        } else {
          console.warn('⚠️  加载 .env 文件失败:', dotenvError.message)
        }
        return false
      }
    }
    ...
    // 加载 .env 失败，静默忽略
    if (process.env.DEBUG === 'true') {
      console.warn('⚠️  检查 .env 文件时出错:', loadError.message)
    }
    return false
  }
}
...
function createMultiStageProgressBar (stages, operation = '操作') {
  ...
  function displayProgress () {
    ...
    // 显示进度信息
    const stageInfo = `${currentStage.emoji} ${currentStage.name
      } [${stageBar}] ${Math.round(currentStageProgress)}%`
    const overallInfo = `总进度: [${progressBar}] ${overallPercent}% (${currentStageIndex + 1
      }/${totalStages})`
...
/**
 * 获取打包文件名
 * 优先级：命令行参数 > npm配置 > 环境变量（.env文件）> 默认值
 ...
 * 获取目标目录
 * 优先级：命令行参数 > npm配置 > 环境变量（.env文件）> 默认值
 * @param {string} defaultValue - 默认目标目录
 * @returns {string} 目标目录路径
 */
function getTargetDir (defaultValue = 'D:/Work/Vue3/yiyumsaas') {
  // 1. 优先从命令行参数获取（最高优先级）
  const args = process.argv.slice(2)
  ...
  // 2. 从 npm config 获取
  const npm_config_target = process.env.npm_config_target
  if (npm_config_target) {
    return npm_config_target
  }
...
/**
 * 获取源目录
 * 优先级：命令行参数 > npm配置 > 环境变量（.env文件）> 构建文件名目录 > 默认值
 * @param {string} defaultValue - 默认源目录（相对于当前工作目录）
 * @returns {string} 源目录路径
 */
function getSourceDir (defaultValue = null) {
  // 1. 优先从命令行参数获取（最高优先级）
  const args = process.argv.slice(2)
  const sourceArg = args.find(arg => arg.startsWith('--source='))
  if (sourceArg) {
    return sourceArg.split('=')[1]
  }

  // 2. 从 npm config 获取源目录
  const npm_config_source = process.env.npm_config_source
  if (npm_config_source) {
    return npm_config_source
  }

  // 3. 检查是否通过 npm run script --build= 指定了构建文件名
  // 如果指定了 npm_config_build，且没有指定 SOURCE_DIR，应该使用构建文件名作为源目录
  const npm_config_build = process.env.npm_config_build
  const hasBuildConfig = npm_config_build !== undefined
  
  // 4. 从环境变量获取（包括从 .env 文件加载的）
  const SOURCE_DIR = process.env.SOURCE_DIR
  
  // 如果通过 npm run script --build= 指定了构建文件名，且没有明确指定 SOURCE_DIR
  // 应该使用构建文件名作为源目录（优先级高于 SOURCE_DIR 环境变量）
  if (hasBuildConfig && !SOURCE_DIR) {
    return npm_config_build
  }
  ...
  // 5. 如果没有指定 SOURCE_DIR，使用构建文件名（BUILD_NAME）对应的目录
  const buildName = getFileName()
  if (buildName) {
    return buildName // 返回构建文件名，调用者会将其作为目录名使用
  }
...
function getAutoConfig () {
  return {
    ...
    useNotification:
      ...
      process.env.USE_NOTIFICATION !== 'false' &&
...
function getEnvConfig () {
  return {
    ...
    useVcsHistory: process.env.USE_VCS_HISTORY !== 'false',
    ...
    retryDelay: (() => {
      const value = process.env.RETRY_DELAY
      if (value === undefined || value === '') return null
      const parsed = parseInt(value, 10)
      return isNaN(parsed) ? null : parsed
    })()
