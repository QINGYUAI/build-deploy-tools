...
async function commitToSvnWithRetry (targetDir, parentDir) {
  return await retryOperation(async () => {
    ...
    await executeSvn('svn commit -m "更新构建文件"', parentDir, 'SVN提交失败', RETRY_CONFIG.commitTimeout, true);
...
async function main () {
  // 显示运行模式
  console.log(`🚀 开始执行构建后复制任务 (最大重试次数: ${RETRY_CONFIG.maxRetries})`);
  console.log(`📋 运行模式: ${AUTO_CONFIG.isAutoMode ? '🤖 自动模式' : '👤 交互模式'}`);
  ...
  return await retryOperation(async () => {
    ...
    const shouldCommit = await confirmAction('是否提交到SVN？', AUTO_CONFIG.autoCommit);
    if (shouldCommit) {
    ...
    console.log(`🎉 操作完成！`);
    return true;
  }, RETRY_CONFIG.maxRetries, RETRY_CONFIG.retryDelay, '整个构建复制流程');
}
